...
### 1. **[SLA-Constrained Carbon-Aware Routing in Geo-Distributed Serverless Clouds](https://arxiv.org/abs/2607.22806v1)**
- **评分**: ⭐ 78/100 | **日期**: 2026-07-24
- **作者**: Anmol Chaudhary, Rahul Mishra
- **标签**: `cs.LG` `cs.DC`
- **AI摘要**: 一种满足SLA约束的碳感知地理分布式无服务器路由策略
<details>
...
- **评分理由**: 在真实云环境评估，工程实用性强，且具备显著节能效果。

### 2. **[Twin-Fidelity-Aware Resolution of Direct xApp Conflicts in Open RAN](https://arxiv.org/abs/2607.22857v1)**
- **评分**: ⭐ 75/100 | **日期**: 2026-07-24
- **作者**: Akram Almohammedi, Mohammed Balfaqih, Sam Darshi et al.
- **标签**: `cs.NI`
- **AI摘要**: 提出一种基于数字孪生保真度的RAN冲突仲裁机制
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 针对O-RAN xApp冲突提出轻量级仲裁算法，系统评估扎实。

## eBPF & Observability

### 1. **[KernelScript: Cross-Boundary Typed DSL for eBPF Applications](https://arxiv.org/abs/2607.23900v1)**
- **评分**: ⭐ 88/100 | **日期**: 2026-07-27
- **作者**: Cong Wang, Siyuan Sun, Yusheng Zheng
- **标签**: `cs.PL` `cs.OS`
- **AI摘要**: KernelScript通过统一DSL解决eBPF跨边界类型安全问题
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 解决了eBPF开发中常见的类型不匹配痛点，工程实用性极高

## Network Stack & Protocol

### 1. **[CAPS: Fine-Tuning CCA Timing](https://arxiv.org/abs/2607.22821v1)**
- **评分**: ⭐ 82/100 | **日期**: 2026-07-24
- **作者**: Raphael Zailer, Isaac Keslassy
- **标签**: `cs.NI`
- **AI摘要**: 一种解耦速率与调度的拥塞控制机制
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 通过ACK同步消除排队，实验验证充分且实用

### 2. **[Fake It No More: Evaluating L4S with SCReAM on Video Traffic](https://arxiv.org/abs/2607.23767v1)**
- **评分**: ⭐ 78/100 | **日期**: 2026-07-26
- **作者**: Nawel Alioua, Ryan Zanone, Cheng Xi et al.
- **标签**: `cs.NI`
- **AI摘要**: 评估L4S对视频流量的性能影响
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 通过仿真验证了L4S在视频传输中的QoE表现

## Middleware & Runtime

### 1. **[Parallel Cascaded Recursive Filtering on Multi-Core CPUs and GPUs](https://arxiv.org/abs/2607.23763v1)**
- **评分**: ⭐ 78/100 | **日期**: 2026-07-26
- **作者**: Haotian Zhai, Bernd-Peter Paris
- **标签**: `eess.SP` `cs.DC`
- **AI摘要**: 多核CPU与GPU上的并行级联递归滤波优化
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 通过叠加原理解决依赖，实现高性能并行计算。

### 2. **[$g$MAGNUS: Fast SpGEMM on GPUs for Irregular Matrices via Hierarchical Multisplit](https://arxiv.org/abs/2607.22866v1)**
- **评分**: ⭐ 78/100 | **日期**: 2026-07-24
- **作者**: Jordi Wolfson-Pou, Ahmed Helal, Fabrizio Petrini
- **标签**: `cs.DC`
- **AI摘要**: GPU稀疏矩阵乘法的高效分层拆分算法
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 针对GPU内存瓶颈的优化，实测性能提升显著

### 3. **[The Atoms of the Score: Record-Level versus In-Engine Composite Evaluation of Clinical Quality Language](https://arxiv.org/abs/2607.23619v1)**
- **评分**: ⭐ 78/100 | **日期**: 2026-07-26
- **作者**: Angelo Kastroulis
- **标签**: `cs.DB` `cs.CE` `cs.PF`
- **AI摘要**: Mercury数据库引擎优化临床质量语言评估性能
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 针对特定领域设计的高性能数据库引擎，实测数据详实且具备实际部署价值。

### 4. **[FlowLog: Re-thinking Datalog for Fast and Extensible Static Analysis](https://arxiv.org/abs/2607.23971v1)**
- **评分**: ⭐ 78/100 | **日期**: 2026-07-27
- **作者**: Zhenghong Yu, Hangdong Zhao, Wanzhu Hou et al.
- **标签**: `cs.PL` `cs.LO` `cs.SE`
- **AI摘要**: 基于差分数据流的静态分析Datalog编译器
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 通过差分数据流实现增量分析，工程实用性强且性能优异。

### 5. **[Order in Desbordante: Techniques for Efficient Implementation of Order Dependency Discovery Algorithms](https://arxiv.org/abs/2607.23632v1)**
- **评分**: ⭐ 75/100 | **日期**: 2026-07-26
- **作者**: Yakov Kuzin, Dmitriy Shcheka, Michael Polyntsov et al.
- **标签**: `cs.DB` `cs.AI` `cs.LG`
- **AI摘要**: 优化数据库顺序依赖发现算法的实现性能
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 通过C++重构与工程优化显著提升了数据分析工具的性能
