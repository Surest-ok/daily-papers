...
### 1. **[TARE: Tail Aware Evaluation of HPC Job Runtime Prediction](https://arxiv.org/abs/2607.04935v1)**
- **评分**: ⭐ 82/100 | **日期**: 2026-07-06
- **作者**: Haili Xiao, Can Wu, Shasha Lu et al.
- **标签**: `cs.DC` `cs.PF`
- **AI摘要**: 提出尾部感知评估法优化HPC作业调度
<details>
...
- **评分理由**: 基于生产环境轨迹的调度策略改进，实用性强且评估严谨

## File Systems & Storage

### 1. **[Cross-IP Request Coalescing: Relocating the Fan-out Point in Virtualized I/O](https://arxiv.org/abs/2607.02969v1)**
- **评分**: ⭐ 82/100 | **日期**: 2026-07-03
- **作者**: Kiseok Kim, Hyeontae Joo, Hwangnam Kim
- **标签**: `cs.DC` `cs.OS`
- **AI摘要**: 通过后端聚合多设备IO请求，降低虚拟化IO开销。
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 针对SPDK虚拟化IO路径的优化，实测性能提升显著。

### 2. **[GORIO: GPU-Centered Remote I/O for Graph ANNS over NVMe-oF](https://arxiv.org/abs/2607.04415v1)**
- **评分**: ⭐ 82/100 | **日期**: 2026-07-05
- **作者**: Gen Zhang, Wenhao Gu
- **标签**: `cs.DC` `cs.DB`
- **AI摘要**: 实现GPU驱动的远程NVMe-oF存储访问以加速图搜索
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 系统设计创新，通过GPU卸载I/O路径显著提升性能

## eBPF & Observability

### 1. **[Characterizing and Bridging the Diagnostic Gap in eBPF Verifier Rejections](https://arxiv.org/abs/2607.02748v1)**
- **评分**: ⭐ 88/100 | **日期**: 2026-07-02
- **作者**: Yusheng Zheng, Zhengjie Ji, Weichen Tao et al.
- **标签**: `cs.OS` `cs.PL` `cs.SE`
- **AI摘要**: 提出bpfix工具以定位eBPF验证器错误根源
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 针对eBPF开发痛点，工具实用且开源，评估严谨。

## Network Stack & Protocol

### 1. **[A Modular O-RAN Testbed Based on SRS Open Source O-CU/O-DU and Massive Beams Modular O-RU](https://arxiv.org/abs/2607.05146v1)**
- **评分**: ⭐ 78/100 | **日期**: 2026-07-06
- **作者**: Fabian Göttsch, Oriol Font-Bach, Andreas Benzin et al.
- **标签**: `cs.NI` `eess.SP`
- **AI摘要**: 基于SRS与MB硬件的模块化O-RAN测试平台
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 系统架构清晰，具备实际部署与实验价值，工程实用性强。

### 2. **[A Binary and System Integrated Analysis Approach for Securing the QUIC Protocol](https://arxiv.org/abs/2607.03149v1)**
- **评分**: ⭐ 78/100 | **日期**: 2026-07-03
- **作者**: Maitha Alshaali, Wanqing Tu, Gaofei Huang et al.
- **标签**: `cs.NI` `cs.CR`
- **AI摘要**: 提出结合二进制与系统级分析的QUIC协议安全评估方法
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 通过实测分析QUIC实现漏洞，具备高实用性与严谨性

### 3. **[Energy-Aware System-Level Evaluation of Post-Quantum TLS on Embedded User Equipment over a Disaggregated 5G Network](https://arxiv.org/abs/2607.03988v1)**
- **评分**: ⭐ 78/100 | **日期**: 2026-07-04
- **作者**: Sanzida Hoque, Abdullah Aydeger
- **标签**: `cs.CR` `cs.PF`
- **AI摘要**: 评估嵌入式设备中PQC TLS的能耗与性能表现
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 基于真实硬件的性能与能耗分析，对5G协议部署有参考价值

### 4. **[HyNoC: A Hybrid Circuit-Switch/Wormhole Network-on-Chip for Distributed VLIW Computing on FPGA](https://arxiv.org/abs/2607.02729v1)**
- **评分**: ⭐ 75/100 | **日期**: 2026-07-02
- **作者**: Christophe Clienti
- **标签**: `cs.AR` `cs.DC`
- **AI摘要**: 提出一种混合交换NoC架构以优化FPGA互联
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 架构设计实用且开源，但在生产环境部署受限

## Middleware & Runtime

### 1. **[Fine-Grained Computation Offload for Off-the-Shelf Servers in Tens of Lines](https://arxiv.org/abs/2607.02630v1)**
- **评分**: ⭐ 88/100 | **日期**: 2026-07-02
- **作者**: Bojie Li
- **标签**: `cs.DC` `cs.AI` `cs.OS`
- **AI摘要**: 利用现有并发机制实现细粒度计算卸载，无需重构代码。
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 通过复用现有服务器并发模型实现卸载，工程落地性极强，评估详实。

### 2. **[Graph-Aware Fuzzing for Graph Database Management Systems](https://arxiv.org/abs/2607.03741v1)**
- **评分**: ⭐ 78/100 | **日期**: 2026-07-04
- **作者**: Yu Li, Qiang Hu, Yao Zhang et al.
- **标签**: `cs.SE`
- **AI摘要**: 提出GRAF图数据库模糊测试框架
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 针对图数据库的测试方法创新，有效发现大量漏洞

### 3. **[Using OAI Overlay to Enhance REST API Fuzzing](https://arxiv.org/abs/2607.04325v1)**
- **评分**: ⭐ 75/100 | **日期**: 2026-07-05
- **作者**: Omur Sahin, Man Zhang, Alexander Poth et al.
- **标签**: `cs.SE`
- **AI摘要**: 利用OAI Overlay增强REST API模糊测试
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 通过扩展EvoMaster实现工业级API测试，实用性强

## Distributed Systems

### 1. **[Cassandra: Consensus with Partial Progress via Robust Partitionable View Synchronization](https://arxiv.org/abs/2607.02856v1)**
- **评分**: ⭐ 82/100 | **日期**: 2026-07-03
- **作者**: Shaokang Xie, Dakai Kang, Junchao Chen et al.
- **标签**: `cs.DC` `cs.DB`
- **AI摘要**: 提出支持分区部分进展的BFT共识协议
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 协议设计创新且评估详实，具备工程落地潜力

### 2. **[Remora: Scale-out Deterministic Execution for Smart Contracts](https://arxiv.org/abs/2607.02817v1)**
- **评分**: ⭐ 82/100 | **日期**: 2026-07-02
- **作者**: Zhengqing Liu, Alberto Sonnino, Igor Zablotchi et al.
- **标签**: `cs.DC` `cs.DB`
- **AI摘要**: 一种支持智能合约扩展的确定性执行引擎
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 架构创新且性能提升显著，具备实际工程落地潜力

### 3. **[Can Code Specify a System Precisely Enough to Formally Verify It?](https://arxiv.org/abs/2607.05076v1)**
- **评分**: ⭐ 75/100 | **日期**: 2026-07-06
- **作者**: Jean-Jacques Dubray
- **标签**: `cs.SE` `cs.FL`
- **AI摘要**: 利用LLM生成契约并验证生产系统协议正确性
<details>
<summary>原始摘要</summary>
...
- **评分理由**: 通过生产环境验证了形式化验证的实用性与局限性

### 4. **[CATs: Secure Blockchain Interoperability with Cross-chain Atomic Transactions](https://arxiv.org/abs/2607.05387v1)**
- **评分**: ⭐ 70/100 | **日期**: 2026-07-06
- **作者**: Andreas Penzkofer, Franck Cassez
- **标签**: `cs.DC`
- **AI摘要**: 提出跨链原子交易协议实现链间互操作性
<details>
<summary>原始摘要</summary>
